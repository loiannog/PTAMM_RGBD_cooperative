...
public:
	...
	TooN::Vector<3> getCenter(const double pos[3], const double att[4], bool rescale_center);
};
...
TooN::Vector<3> AxesArray::getCenter(const double pos[3], const double att[4], bool rescale_center)
{
	if(rescale_center){
...
/*
	...
	*/
	TooN::Matrix<3> Rot;
		...
		double y = q[2];
		double z = q[3];
		TooN::Fill(Rot) = 1-2*(y*y+z*z),2*(x*y-s*z),2*(x*z+s*y),
