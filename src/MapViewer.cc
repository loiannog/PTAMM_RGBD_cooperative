...
void MapViewer::DrawMap(SE3<> se3CamFromWorld)
{
  ...
  if( mpViewingMap == mpMap ) {
    ...
    //DrawCamera(se3CamFromWorld2);
  }
  
  for(size_t i=0; i<mpViewingMap->vpKeyFrames.size(); i++){
    DrawCamera(TooN::SE3<> (mpViewingMap->vpKeyFrames[i]->se3CfromW.get_rotation(),mpViewingMap->vpKeyFrames[i]->scale_factor_key_frame*mpViewingMap->vpKeyFrames[i]->se3CfromW.get_translation()), true);
  }
