...
void MapMaker::run()
{
  ...
  while(!shouldStop())  // ShouldStop is a CVD::Thread func which return true if the thread is told to exit.
	...
	//BundleAdjustRecent();
    /////////////////////////////////////Modified Loianno//////////////////////////////////////////////////
		if (!mpMap->bBundleConverged_Recent && mpMap->QueueSize() == 0  &&(ParamsAccess::varParams->BundleMethod=="LOCAL" || ParamsAccess::varParams->BundleMethod=="LOCAL_GLOBAL"))
			BundleAdjustRecent();
   ///////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//BundleAdjustAll();
      /////////////////////////////////////Modified Loianno//////////////////////////////////////////////////
  		if (mpMap->bBundleConverged_Recent && !mpMap->bBundleConverged_Full && mpMap->QueueSize() == 0 && (ParamsAccess::varParams->BundleMethod=="GLOBAL" || ParamsAccess::varParams->BundleMethod=="LOCAL_GLOBAL"))
  			BundleAdjustAll();
     ///////////////////////////////////////////////////////////////////////////////////////////////////////
      CKECK_ABORTS;
...
/*  AddSomeMapPoints(0);
  ...
  ////////////////////////////////modified Loianno///////////////////////////////////////
	...
	if(!ParamsAccess::varParams->NoLevelZeroMapPoints)
 ...
 /*
  ...
  //////////////////////////////////modified Loianno//////////////////////////////////////
	double nloops=0;
	while(!mpMap->bBundleConverged_Full & (nloops<ParamsAccess::varParams->MaxStereoInitLoops))
	{
		BundleAdjustAll();
		if(mbResetRequested || mbReInitRequested || mbSwitchRequested || mbResetRequested || (nloops>=ParamsAccess::varParams->MaxStereoInitLoops))
			return false;
		nloops++;
	}
///////////////////////////////////////modified Loianno////////////////////////////////////////////////////////////
...
// InitFromStereo() generates the initial match from two keyframes
// and a vector of image correspondences. Uses the
bool MapMaker::InitFromStereoDepth(KeyFrame &kF,
			      vector<ImageRef> &vTrailMatches,  pcl::PointCloud<pcl::PointXYZ> &pt,
			      SE3<> &se3TrackerPose)
{
  mdWiggleScale = *mgvdWiggleScale; // Cache this for the new map.
  ATANCamera &Camera = kF.Camera;
  ...
  for(unsigned int i=0; i<vTrailMatches.size(); i++)
    {
      ...
      finder.MakeTemplateCoarseNoWarp(*p);
      finder.MakeSubPixTemplate();
      finder.SetSubPixPos(vec(vTrailMatches[i]));
      //bool bGood = finder.IterateSubPixToConvergence(*pkSecond,10);
    //  if(!bGood)
	//{
	  //delete p; continue;
	//}
      ...
      Vector<2> v2SecondPos = finder.GetSubPixPos();
      //p->v3WorldPos = ReprojectPoint(se3, Camera.UnProject(v2SecondPos), vMatches[i].v2CamPlaneFirst);
		p->v3WorldPos[0] = pt[i].x;
		p->v3WorldPos[1] = pt[i].y;
		p->v3WorldPos[2] = pt[i].z;
      if(p->v3WorldPos[2] < 0.0)
       	{
 	 delete p; continue;
 	}
      ...
      mFirst.nLevel = 0;
      mFirst.Source = Measurement::SRC_ROOT;
      mFirst.v2RootPos = vec(vTrailMatches[i]);
      mFirst.v2ImplanePos = Camera.UnProject(mFirst.v2RootPos);
      mFirst.m2CamDerivs = Camera.GetProjectionDerivs();
      mFirst.bSubPix = true;
      pkFirst->mMeasurements[p] = mFirst;//assign the keyframe to the corresponding 3D point
      p->pMMData->sMeasurementKFs.insert(pkFirst);
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////
	 //inital rotation for visualization
	  TooN::Matrix<3, 3, double> R2 =   TooN::Data( -1,   0,     0,
			                                         0,   1,     0,
			                                         0 ,  0 ,   -1);
	  TooN::Matrix<3, 3, double> rot_temp = TooN::Identity;
	  TooN::Vector<3, double>  trans_temp = TooN::makeVector(0.0,0.0, 0.0);
	  TooN::SE3<double> H_tot(rot_temp,trans_temp);
  // Rotate and translate the map so the dominant plane is at z=0:

  Matrix<3> R_temp = TooN::Identity;
  SO3<double> R(R_temp);
  Vector<3> t = TooN::makeVector(0,0,0);
  SE3<double> H(R,t);
  pkFirst->se3CfromW = H;
  pkFirst->scale_factor_key_frame = 1;
  se3TrackerPose = H_tot*pkFirst->se3CfromW;
  ApplyGlobalTransformationToMap(se3TrackerPose);
  mpMap->bGood = true;
  cout << "  MapMaker: made initial map with " << mpMap->vpPoints.size() << " points." << endl;
  return true;
}
 ...
 */
void MapMaker::AddKeyFrame(KeyFrame &k)//modified Loianno
{
...
void MapMaker::AddKeyFrameFromTopOfQueue()
{
	...
	ParamsAccess Params;
	PTAMM_RGBD_cooperative::PtammParamsConfig* pPars = Params.varParams;
	if (pPars->MaxKF>1)	// MaxKF<2 means keep all KFs
	{
		while (mpMap->vpKeyFrames.size()>(unsigned int)pPars->MaxKF)
		{
			// find farthest KF
			double dFarthestDist = 0;
			vector<KeyFrame*>::iterator itFarthest =  mpMap->vpKeyFrames.begin();
			...
			for (unsigned int i=0;i< mpMap->vpPoints.size();i++)
				if ( mpMap->vpPoints[i]->pPatchSourceKF==*itFarthest)
					 mpMap->vpPoints[i]->bBad=true;

			if ((*itFarthest)->bFixed)	// if we delete the a fix KF, fix the oldest one
			{
				 mpMap->vpKeyFrames.erase(itFarthest);
				 mpMap->vpKeyFrames[0]->bFixed=true;
			}
			else
				 mpMap->vpKeyFrames.erase(itFarthest);
		}
	...
	AddSomeMapPoints(2);*/
	/////////////////////////////modified Loianno////////////////////////////
	   // .. and add more map points by epipolar search.
	bool addedsome=false;
	addedsome |= AddSomeMapPoints(3);       // .. and add more map points by epipolar search.
	if(!ParamsAccess::varParams->NoLevelZeroMapPoints)
		addedsome |= AddSomeMapPoints(0);
...
bool MapMaker::NeedNewKeyFrame(KeyFrame &kCurrent)
{
  ...
  /////////////////////////////modified Loianno//////////////
  if(dDist > GV2.GetDouble("MapMaker.MaxKFDistWiggleMult",ParamsAccess::varParams->MaxKFDistWiggleMult,SILENT) * mdWiggleScaleDepthNormalized)
  ///////////////////////////////////////////////////////////
    return true;
  return false;
}
...
SE3<> MapMaker::CalcPlaneAligner()
{
  ...
  ///////////////////////////////////modified Loianno///////////////////////////
  int nRansacs = GV2.GetInt("MapMaker.PlaneAlignerRansacs", ParamsAccess::varParams->PlaneAlignerRansacs, HIDDEN|SILENT);
