...
//  GUI.RegisterCommand("KeyPress", GUICommandCallBack, this);
  ...
  mpSBILastFrame = NULL;
  mpSBIThisFrame = NULL;
  depth_initialization = true;//modified Loianno
...
void Tracker::TrackFrame(Image<CVD::byte> &imFrame, bool bDraw, const sensor_msgs::ImageConstPtr & depth_msg)//last parameter is optional
{
  ...
  ///////////////////////////////////modified Loianno/////////////////////////////////
  static gvar3<double> gvdSBIBlur("Tracker.RotationEstimatorBlur", ParamsAccess::varParams->RotationEstimatorBlur, SILENT);
  static gvar3<int> gvnUseSBI("Tracker.UseRotationEstimator", ParamsAccess::varParams->UseRotationEstimator, SILENT);
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  if(mpMap->IsGood())
      ...
      if(mnLostFrames < NUM_LOST_FRAMES)  // .. but only if we're not lost!
	{
	  ...
	  if(mTrackingQuality == GOOD &&
	     ...
	     //mnFrame - mnLastKeyFrameDropped > 20  &&//modified Loianno
	     mpMap->QueueSize() < 3)
	    {
	      mMessageForUser << " Adding key-frame.";
	      //if(counter==0){
	    	//  mFirstKF.scale_factor_key_frame = scale_factor;
	    	  //counter = 1;
	      //}
	      //mCurrentKF.scale_factor_key_frame = scale_factor;
	      AddNewKeyFrame();
	      current_key_Frame_added_to_map = true;//modified Loianno check if the current Keyframe has been added
	    };
  ...
  else if(!mpMap->IsGood() && depth_initialization)
	  TrackForInitialDepth(depth_msg);
  else // If there is no map, try to make one.
    TrackForInitialMap();

	  ///////////////renitilization if the map is lost/////////////////////////
/*		ParamsAccess Params;
...
///////////////////////////////////////modified Loianno//////////////////////////////////////////////
/**
 * Routine for establishing the initial map usng the depth
 */
void Tracker::TrackForInitialDepth(const sensor_msgs::ImageConstPtr & depth_msg)
{
  ...
  static gvar3<int> gvnMaxSSD("Tracker.MiniPatchMaxSSD", ParamsAccess::varParams->MiniPatchMaxSSD, SILENT);
  ////////////////////////////////////////////////////////////////////////////////////////
  ...
  //if(mnInitialStage == TRAIL_TRACKING_STARTED)
     ...
     if(mbUserPressedSpacebar)
	{
	  ...
	  mbUserPressedSpacebar = false;
		...
		vector<ImageRef> vMatches;   // This is the format the mapmaker wants for the stereo pairs
		 pcl::PointCloud<pcl::PointXYZ> pt;
		 int counter_point_cloud = 0;
		for(list<Trail>::iterator i = mlTrails.begin(); i!=mlTrails.end(); i++){
			//take the initial extracted features combine with the depth
			int u = LevelZeroPos(i->irInitialPos.x, level);//pixel x
			int v = LevelZeroPos(i->irInitialPos.y, level);//pixel y
  		...
  		// Find XYZ
  			ImageRef tmp(u,v);
  			vMatches.push_back(tmp);
  			pt.resize(counter_point_cloud+1);
  			pt[counter_point_cloud].x = (u - ParamsAccess::fixParams->cx_rgb) * depth * constant_x_rgb;
  			pt[counter_point_cloud].y = (v - ParamsAccess::fixParams->cy_rgb) * depth * constant_y_rgb;
  			pt[counter_point_cloud].z = depth_image_proc::DepthTraits<float>::toMeters(depth);
  			counter_point_cloud++;

  		}
		}
  		//add correct point to the map
	  mMapMaker.InitFromStereoDepth(mCurrentKF, vMatches, pt, mse3CamFromWorld);  // This will take some time!
	  ///////////////////////////modified Loianno/////////////////////////////////////////////////
	  ...
	  //trail_sempahore = true;//modified Loianno
	  mnInitialStage = TRAIL_TRACKING_COMPLETE;
	  trail_sempahore = true;//modified Loianno
	  ROS_INFO_STREAM("Initialized map on level "<<level);
	  current_key_Frame_added_to_map = true;
	}//end stereo initialization
      //else
...
void Tracker::TrackMap()
{
  ...
  for(int i=0; i<LEVELS; i++)
  {
    //cout<<"Level: " << i << ", num_pts: " << avPVS[i].size()<<endl;  
  ...
  ////////////////////////modified Loianno//////////////////////
  static gvar3<unsigned int> gvnCoarseMin("Tracker.CoarseMin", ParamsAccess::varParams->CoarseMin, SILENT);   // Min number of large-scale features for coarse stage
  static gvar3<unsigned int> gvnCoarseMax("Tracker.CoarseMax", ParamsAccess::varParams->CoarseMax, SILENT);   // Max number of large-scale features for coarse stage
  static gvar3<unsigned int> gvnCoarseRange("Tracker.CoarseRange", ParamsAccess::varParams->CoarseRange, SILENT);       // Pixel search radius for coarse features
  static gvar3<int> gvnCoarseSubPixIts("Tracker.CoarseSubPixIts", ParamsAccess::varParams->CoarseSubPixIts, SILENT); // Max sub-pixel iterations for coarse features
  static gvar3<int> gvnCoarseDisabled("Tracker.DisableCoarse", ParamsAccess::varParams->DisableCoarse, SILENT);    // Set this to 1 to disable coarse stage (except after recovery)
  static gvar3<double> gvdCoarseMinVel("Tracker.CoarseMinVelocity", ParamsAccess::varParams->CoarseMinVelocity, SILENT);  // Speed above which coarse stage is used.
  ...
  //////////////////////////////modified Loianno/////////////////////////////////////
  static gvar3<int> gvnMaxPatchesPerFrame("Tracker.MaxPatchesPerFrame", ParamsAccess::varParams->MaxPatchesPerFrame, SILENT);
  ////////////////////////////////////////////////////////////////////////////////////
  int nFinePatchesToUse = *gvnMaxPatchesPerFrame - static_cast<int>(vIterationSet.size());
  //if((int) vNextToSearch.size() > nFinePatchesToUse)//modified Loianno
 ...
 //cout<<"vNextToSearch_1:"<<vNextToSearch.size()<<endl;
  ...
  for(int iter = 0; iter<10; iter++)
	...
	bNonLinearIteration = true;   // Even tvNextToSearchhis is probably overkill, the reason we do many
...
Vector<6> Tracker::CalcPoseUpdate(vector<TrackerData*> vTD, double dOverrideSigma, bool bMarkOutliers)
{
  ...
  /////////////////////////////modified Loianno//////////////////////////
  static gvar3<string> gvsEstimator("TrackerMEstimator", ParamsAccess::fixParams->TrackerMEstimator, SILENT);
 ////////////////////////////////////////////////////////////////////////
  ...
  //////////////////////////////////modified Loianno///////////////////////////////////////
	...
	//cout<<"mmCovariances:"<<mmCovariances<<endl;
  /////////////////////////////////////////////////////////////////////////////////////////
  return wls.get_mu();
}
///////////////////////////modified Loianno/////////////////////////////////
void  Tracker::mean_variance(vector <double> scale_fact, int size){
	double mean = 0;
	int SOGLIA_STD = 2.5;
	for(int i = 0; i< scale_fact.size(); i++)
	mean = mean + scale_fact[i];
	mean = mean/scale_fact.size();
	vector <double> sqrt_variance;
	sqrt_variance.clear();
	double standard_deviation = 0;
	for(int i = 0; i< scale_fact.size(); i++){
		sqrt_variance.resize(i+1);
         sqrt_variance[i] =  (scale_fact[i] - mean)*(scale_fact[i] - mean);
         standard_deviation = standard_deviation + sqrt_variance[i];

	}
	standard_deviation = sqrt(standard_deviation/scale_fact.size());
int punti_scartati = 0;
    //erase elements outside standard deviation coefficient
	for (int i = 0; i< scale_fact.size(); i++){
		if(abs(scale_fact[i] - mean) > SOGLIA_STD*standard_deviation){
			scale_fact.erase(scale_fact.begin()+i);
			i--;
			punti_scartati++;

	}
	}
	if(size != scale_fact.size()){
		mean_variance(scale_fact, scale_fact.size());
}
	else{
		scale_factor_final.resize(scale_fact.size());
		for (int i = 0; i<scale_fact.size(); i++){
			scale_factor_final[i] = scale_fact[i];
		}
		return;
	}
}
...
void Tracker::AddNewKeyFrame()
{
  ...
  mCurrentKF.scale_factor_key_frame = scale_factor_final_scalar_tracker;//modified Loianno
...
void Tracker::AssessTrackingQuality()
{
  ...
  if(nTotalFound == 0 || nTotalAttempted == 0)
   ...
   ////////////////////////////////////modified Loianno///////////////////////////////
      static gvar3<double> gvdQualityGood("Tracker.TrackingQualityGood", ParamsAccess::varParams->TrackingQualityGood, SILENT);
      static gvar3<double> gvdQualityLost("Tracker.TrackingQualityLost", ParamsAccess::varParams->TrackingQualityLost, SILENT);
      /////////////////////////////////////////////////////////////////////////////////
